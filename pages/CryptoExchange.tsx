...
/**
 * ZENTUM CRYPTO EXCHANGE - PROFESSIONAL ULTIMATE EDITION (V4.9)
 * -----------------------------------------------------------
...
const CryptoExchange: React.FC<CryptoProps> = ({ user, onUpdateBalance, onSyncUserData, onLogout }) => {
  ...
  const handleTrade = async (type: 'BUY' | 'SELL') => {
    ...
    if (price === 0) {
      alert("⚠️ Synchronizing market data feed... please wait.");
      return;
    }
    ...
    if (type === 'BUY') {
      ...
      alert(`PURCHASE CONFIRMED: Bought ${volume} ${selected} at $${price.toLocaleString()}`);
      ...
      if (assetIdx > -1) {
        ...
        alert(`POSITION CLOSED: Result ${netProfit >= 0 ? '+' : ''}${netProfit.toFixed(2)} USDT Realized.`);
      } else {
  ...
  return (
    <div className="h-screen flex flex-col bg-[#0b0e11] text-white overflow-hidden font-sans select-none text-[11px]">
      ...
      <nav className="h-16 border-b border-white/5 bg-[#181a20] flex items-center justify-between px-4 z-[100] shadow-2xl shrink-0">
        ...
        <div className="flex items-center gap-6 sm:gap-12">
          ...
          <div className="flex items-center gap-4 sm:gap-10">
            <button onClick={() => navigate('/')} className="text-gray-400 hover:text-white transition-all uppercase font-black text-[12px] sm:text-[14px] tracking-widest flex items-center gap-2">
            ...
            <button onClick={() => setIsAccountOpen(true)} className="text-gray-400 hover:text-white transition-all uppercase font-black text-[12px] sm:text-[14px] tracking-widest flex items-center gap-2">
        ...
        <div className="flex gap-3 sm:gap-6 items-center">
          <div className="bg-black/40 px-3 py-1.5 rounded-2xl border border-white/5 font-bold uppercase flex items-center gap-2 sm:gap-6 shadow-inner">
              <div className="flex flex-col items-end">
                <span className="text-gray-600 text-[8px] tracking-[0.2em] font-black uppercase">Available Balance</span>
                <span className="text-yellow-500 font-mono text-[14px] sm:text-[16px] font-black tracking-tighter">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2.5">
            <button onClick={() => setIsDepositOpen(true)} className="bg-yellow-600 text-black px-4 sm:px-8 py-2 sm:py-3 rounded-2xl font-black text-[10px] sm:text-[12px] uppercase shadow-lg shadow-yellow-900/30 hover:bg-yellow-500 transition-all active:scale-95">Deposit</button>
            <button onClick={() => setIsWithdrawOpen(true)} className="border border-white/20 text-white px-4 sm:px-8 py-2 sm:py-3 rounded-2xl font-black text-[10px] sm:text-[12px] uppercase hover:bg-white/10 active:scale-95 transition-all">Withdraw</button>
          </div>
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden relative">
        ...
        {/* --- SIDEBAR: Market Watch Strip (Mobile: Horizontal Scroll / Desktop: Vertical List) --- */}
        <div className="w-full md:w-64 border-r border-[#2b2f36] bg-[#1e2329] flex flex-row md:flex-col overflow-x-auto md:overflow-y-auto shrink-0 custom-scrollbar z-10 shadow-xl">
          <div className="hidden md:block p-5 text-[11px] text-gray-500 font-black uppercase border-b border-white/5 tracking-[0.2em] bg-black/10">Market Assets</div>
          {coins.map(c => (
            <div 
              key={c.s} 
        ...
        <div className="flex-1 flex flex-col overflow-hidden min-w-0 bg-[#0b0e11]">
          ...
          <div className="p-3 md:p-4 bg-[#181A20] border-b border-white/5 flex justify-between items-center gap-4 shadow-md">
            <div className="flex flex-col">
              <h2 className="text-lg md:text-xl font-black uppercase tracking-tighter text-white italic leading-none">{selected} <span className="text-gray-500 not-italic">/ USDT</span></h2>
              <span className="text-[7px] md:text-[9px] text-yellow-500 font-black tracking-widest uppercase mt-1">Direct Exchange Active</span>
            </div>

            <div className="flex items-center gap-4">
               <div className="flex items-center bg-black/60 rounded-2xl border border-white/10 h-10 md:h-12 px-5 shadow-inner">
               ...
               <div className="flex bg-[#1e2329] border border-white/10 rounded-2xl h-10 md:h-12 overflow-hidden shadow-2xl">
                  <button onClick={() => handleTrade('SELL')} className="px-10 md:px-16 bg-red-600/20 text-red-500 border-r border-white/5 font-black uppercase text-[11px] md:text-[13px] hover:bg-red-600 hover:text-white transition-all tracking-widest active:scale-95">Sell</button>
                  <button onClick={() => handleTrade('BUY')} className="px-10 md:px-16 bg-[#02c076]/20 text-[#02c076] font-black uppercase text-[11px] md:text-[13px] hover:bg-[#02c076] hover:text-white transition-all tracking-widest active:scale-95">Buy</button>
               </div>
            </div>
          </div>

          {/* Full-width High Definition Chart */}
          <div className="flex-1 bg-black relative shadow-inner">
            <iframe 
              ...
              title="ZENTUM Real-time Cloud Engine" 
            />
          </div>

          {/* --- MT5 STYLE BOTTOM TERMINAL PANEL --- */}
          <div className="h-60 md:h-72 bg-[#181a20] border-t border-[#2b2f36] flex flex-col overflow-hidden shrink-0 shadow-[0_-10px_30px_rgba(0,0,0,0.5)]">
             ...
             <div className="flex bg-[#0b0e11] text-[9px] text-gray-500 border-b border-[#2b2f36] font-black uppercase tracking-widest">
                <button onClick={() => setBottomTab('ASSETS')} className={`px-12 py-4 border-r border-[#2b2f36] transition-all ${bottomTab === 'ASSETS' ? 'bg-[#1e2329] text-yellow-500 font-black' : 'hover:text-white'}`}>Active Portfolio</button>
                <button onClick={() => setBottomTab('HISTORY')} className={`px-12 py-4 border-r border-[#2b2f36] transition-all ${bottomTab === 'HISTORY' ? 'bg-[#1e2329] text-yellow-500 font-black' : 'hover:text-white'}`}>Trade History</button>
             </div>
             ...
             <div className="flex-1 overflow-y-auto custom-scrollbar p-3 md:p-5 bg-[#181a20]">
                <table className="w-full text-left text-white border-collapse">
                   <thead className="text-gray-600 border-b border-white/5 uppercase font-black tracking-widest text-[9px]">
                     <tr>
                        ...
                        <th className="text-right pb-4">{bottomTab === 'ASSETS' ? 'Floating P/L' : 'Closed Result'}</th>
                     </tr>
                   </thead>
                         ...
                         return (
                           <tr key={h.id} className="hover:bg-white/[0.01] transition-colors group">
                             ...
                             <td className={`text-right py-5 font-black font-mono text-base ${pl >= 0 ? 'text-[#02c076]' : 'text-[#f6465d]'}`}>
                               {pl >= 0 ? '+' : ''}{pl.toFixed(2)} <span className="text-[10px] opacity-40">USDT</span>
                             </td>
                           </tr>
                         );
                       })
                ...
                {(bottomTab === 'ASSETS' ? holdings : history).length === 0 && (
                  <div className="p-20 text-center text-gray-800 font-black uppercase tracking-[0.4em] italic opacity-30">No Active Cloud Records</div>
                )}
             </div>

             {/* Dynamic Status Bar */}
             <div className="p-2.5 bg-[#0b0e11] border-t border-white/5 flex gap-10 text-[9px] text-gray-600 font-black uppercase tracking-widest overflow-x-auto whitespace-nowrap">
                <div>Account Integrity: <span className="text-green-500 italic">SECURE</span></div>
                <div>Node Link: <span className="text-white">VERIFIED CLOUD 01</span></div>
                <div className="ml-auto opacity-30 font-mono italic uppercase">ZENTUM CLOUD INFRASTRUCTURE V4.9</div>
             </div>
          </div>
        </div>
      </div>
