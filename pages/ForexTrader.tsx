  ...
  return (
    <div className="h-screen flex flex-col bg-[#0b0e11] text-[#d1d4dc] text-[11px] overflow-hidden font-sans select-none">
      ...
      <nav className="h-16 border-b border-[#2b2f36] bg-[#181a20] flex items-center justify-between px-4 z-[100]">
        ...
        {/* جهة اليمين: Balance + Add Funds + Withdraw + Logout */}
        <div className="flex gap-4 items-center">
           <div className="bg-black/40 px-3 py-1 rounded border border-white/5 font-bold uppercase text-[10px]">
           ...
           <div className="flex gap-2">
             <button onClick={() => setIsDepositOpen(true)} className="bg-blue-600 text-white px-4 py-2 rounded-xl font-black text-[11px] uppercase shadow-lg hover:bg-blue-500 transition-all">Add Funds</button>
             <button onClick={() => alert("Withdrawal request submitted.")} className="border border-white/20 text-white px-4 py-2 rounded-xl font-black text-[11px] uppercase hover:bg-white/10 transition-all">Withdraw</button>
           </div>
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden relative">
        ...
        {/* Market Watch - Mobile Responsive Horizontal Strip */}
        ...
        <div className="flex-1 flex flex-col overflow-hidden min-w-0">
          ...
          <div className="p-3 bg-[#181a20] border-b border-[#2b2f36] flex justify-between items-center gap-2">
            ...
            <div className="flex items-center gap-3">
               ...
               <div className="flex bg-[#1e2329] border border-[#2b2f36] rounded-xl h-10 overflow-hidden shadow-2xl">
                  <button onClick={() => openOrder('SELL')} className="px-8 bg-red-600/20 text-red-500 border-r border-[#2b2f36] font-black text-[11px] uppercase hover:bg-red-600 hover:text-white transition-all">Sell</button>
                  <button onClick={() => openOrder('BUY')} className="px-8 bg-blue-600/20 text-blue-500 font-black text-[11px] uppercase hover:bg-blue-600 hover:text-white transition-all">Buy</button>
               </div>
            </div>
          </div>
          ...
          <div className="h-48 md:h-52 bg-[#181a20] border-t border-[#2b2f36] flex flex-col overflow-hidden shrink-0">
            <div className="flex bg-[#1e2329] text-[9px] text-gray-500 border-b border-[#2b2f36] font-black uppercase tracking-widest">
               <button onClick={() => setBottomTab('TRADE')} className={`px-8 py-3 border-r border-[#2b2f36] transition-all ${bottomTab === 'TRADE' ? 'bg-[#2b2f36] text-white' : 'hover:text-white'}`}>Active Trade</button>
               <button onClick={() => setBottomTab('HISTORY')} className={`px-8 py-3 border-r border-[#2b2f36] transition-all ${bottomTab === 'HISTORY' ? 'bg-[#2b2f36] text-white' : 'hover:text-white'}`}>Trade History</button>
            </div>

            <div className="p-2 bg-[#0b0e11] border-b border-[#2b2f36] flex gap-5 text-[10px] text-gray-400 font-bold uppercase overflow-x-auto whitespace-nowrap">
            ...
            <div className="flex-1 overflow-auto custom-scrollbar">
               <table className="w-full text-left text-white text-[10px]">
                <thead className="text-gray-500 border-b border-white/5 sticky top-0 bg-[#181a20]">
                  <tr><th className="p-2 uppercase font-black tracking-widest">Symbol</th><th>Type</th><th>P/L</th><th className="text-right p-2">{bottomTab === 'TRADE' ? 'Close' : 'Date'}</th></tr>
                </thead>
                <tbody>
                  {bottomTab === 'TRADE' ? (
                    orders.map((o: any) => (
