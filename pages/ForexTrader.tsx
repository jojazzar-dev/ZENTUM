...
import Logo from '../components/Logo'; // استيراد اللوجو كملف مستقل
...
/**
 * ZENTUM FOREX TERMINAL - PROFESSIONAL ULTIMATE EDITION (V4.0)
 * ----------------------------------------------------------------
 ...
 * - Mobile Portrait Compatibility (Horizontal Scrolling Sidebar)
 */
...
const ForexTrader: React.FC<ForexProps> = ({ user, onUpdateBalance, onSyncUserData, onLogout }) => {
  ...
  const [volume, setVolume] = useState(0.10); // Default Lot size
  ...
  // --- [2] محرك الأسعار اللحظية (Real-time Calculation Engine) ---
  function getLivePrice(sym: string) {
    if (!fxRates || Object.keys(fxRates).length === 0) return 1.00000;
    
    // أسعار الذهب والعملات الرقمية داخل الفوركس
    ...
    const microTick = (Math.random() - 0.5) * 0.0001;
    return price + microTick; 
  }

  // --- [3] المحرك المالي المتقدم (الرافعة 5000 واللوت المتغير) ---
  const calculatePL = (o: any) => {
    ...
    // العقد القياسي 100,000 للعملات و 1,000 للذهب والين
  ...
  // حساب المارجن المحجوز: رافعة 1:5000 تعني (لوت 1.0 يحجز فقط 20 دولار من الرصيد)
  ...
  return (
    <div className="h-screen flex flex-col bg-[#0b0e11] text-[#d1d4dc] text-[11px] overflow-hidden font-sans select-none">
      ...
      <nav className="h-16 border-b border-white/5 bg-[#181a20] flex items-center justify-between px-6 z-[100] shadow-2xl shrink-0">
        ...
        <div className="flex items-center gap-12">
          <div className="flex items-center gap-2 cursor-pointer group" onClick={() => navigate('/')}>
            ...
            <span className="font-black text-white uppercase text-sm tracking-[0.2em] italic">ZENTUM</span>
          </div>
        ...
        <div className="flex gap-6 items-center">
           <div className="bg-black/40 px-5 py-2.5 rounded-2xl border border-white/5 font-bold uppercase flex items-center gap-6 shadow-inner">
              <div className="flex flex-col items-end border-r border-white/10 pr-6">
                <span className="text-gray-600 text-[9px] tracking-[0.2em] font-black">ESTIMATED EQUITY</span>
                <span className={`font-mono text-[16px] font-black ${totalPL >= 0 ? 'text-green-500' : 'text-red-500'}`}>${equity.toFixed(2)}</span>
              </div>
              <div className="flex flex-col items-end">
                <span className="text-gray-600 text-[9px] tracking-[0.2em] font-black">WALLET BALANCE</span>
                <span className="text-white font-mono text-[16px] font-black">${user.forexBalance.toFixed(2)}</span>
              </div>
           </div>
           
           <div className="flex gap-2.5">
             <button onClick={() => setIsDepositOpen(true)} className="bg-blue-600 text-white px-7 py-3 rounded-2xl font-black text-[12px] uppercase shadow-lg shadow-blue-900/40 hover:bg-blue-500 transition-all active:scale-95">Add Funds</button>
           ...
           <button onClick={onLogout} className="text-gray-500 hover:text-red-500 ml-3 p-2 transition-colors">
      ...
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden relative">
        ...
        {/* Sidebar: Market Stream (Responsive) */}
        <div className="w-full md:w-64 border-r border-[#2b2f36] bg-[#1e2329] flex flex-row md:flex-col overflow-x-auto md:overflow-y-auto shrink-0 custom-scrollbar z-10 shadow-xl">
          ...
          {pairs.map(s => (
            <div 
              ...
              className={`p-4 md:p-6 border-r md:border-r-0 md:border-b border-white/[0.02] cursor-pointer whitespace-nowrap transition-all ${selected === s ? 'bg-blue-600/10 border-b-2 border-b-blue-500 md:border-b-0 md:border-l-4 md:border-l-blue-500 shadow-inner' : 'hover:bg-white/[0.02]'}`}
            >
              <div className="flex flex-col md:flex-row md:justify-between items-start md:items-center">
                <span className="font-black text-[12px] md:text-[14px] uppercase text-white tracking-tighter">{s}</span>
                <span className="text-blue-500 font-mono text-[10px] mt-1 md:mt-0 opacity-80 font-bold">{getLivePrice(s).toFixed(s.includes('JPY') ? 3 : 5)}</span>
              </div>
            </div>
          ))}
        </div>

        {/* Trade Area: Bar + Chart + Terminal */}
        <div className="flex-1 flex flex-col overflow-hidden min-w-0 bg-[#0b0e11]">
          
          {/* Quick Execution Bar */}
          <div className="p-4 bg-[#181a20] border-b border-[#2b2f36] flex justify-between items-center gap-4 shadow-md">
            <div className="flex flex-col">
              <h2 className="text-xl md:text-2xl font-black uppercase tracking-tighter text-white italic">{selected} <span className="text-gray-600 not-italic">/ USD</span></h2>
              <span className="text-[9px] text-blue-500 font-black tracking-[0.3em] uppercase">Cloud Execution Active</span>
            </div>

            <div className="flex items-center gap-4">
               <div className="flex items-center bg-black/60 rounded-2xl border border-white/10 h-12 px-5 shadow-inner">
                  <span className="text-[10px] text-gray-500 font-black mr-4 uppercase tracking-widest">LOT SIZE</span>
                  <input 
                    type="number" 
                    value={volume} 
               ...
               <div className="flex bg-[#1e2329] border border-[#2b2f36] rounded-2xl h-12 overflow-hidden shadow-2xl">
                  <button onClick={() => openOrder('SELL')} className="px-10 md:px-16 bg-red-600/20 text-red-500 border-r border-[#2b2f36] font-black text-[13px] uppercase hover:bg-red-600 hover:text-white transition-all tracking-widest active:scale-95">Sell</button>
                  <button onClick={() => openOrder('BUY')} className="px-10 md:px-16 bg-blue-600/20 text-blue-500 font-black text-[13px] uppercase hover:bg-blue-600 hover:text-white transition-all tracking-widest active:scale-95">Buy</button>
               </div>
            </div>
          </div>
          ...
          {/* --- [C] MT5 BOTTOM TERMINAL --- */}
          <div className="h-60 md:h-72 bg-[#181a20] border-t border-[#2b2f36] flex flex-col overflow-hidden shrink-0 shadow-[0_-10px_30px_rgba(0,0,0,0.5)]">
            
            {/* Terminal Tab Selector */}
            <div className="flex bg-[#0b0e11] text-[10px] text-gray-500 border-b border-[#2b2f36] font-black uppercase tracking-widest">
               <button onClick={() => setBottomTab('TRADE')} className={`px-12 py-4 border-r border-[#2b2f36] transition-all ${bottomTab === 'TRADE' ? 'bg-[#1e2329] text-blue-500 font-black' : 'hover:text-white'}`}>Active Positions</button>
               <button onClick={() => setBottomTab('HISTORY')} className={`px-12 py-4 border-r border-[#2b2f36] transition-all ${bottomTab === 'HISTORY' ? 'bg-[#1e2329] text-blue-500 font-black' : 'hover:text-white'}`}>Account History</button>
            </div>

            {/* Terminal Account Bar */}
            <div className="p-3 bg-[#0b0e11] border-b border-[#2b2f36] flex gap-8 text-[11px] text-gray-400 font-black uppercase overflow-x-auto whitespace-nowrap custom-scrollbar">
               ...
               <div className="flex gap-2">Free Margin: <span className={`font-mono ${freeMargin < 0 ? 'text-red-500' : 'text-green-400'}`}>${freeMargin.toFixed(2)}</span></div>
            ...
            <div className="flex-1 overflow-auto custom-scrollbar bg-[#181a20] p-4">
               <table className="w-full text-left text-white text-[11px] border-collapse">
                <thead className="text-gray-600 border-b border-white/5 sticky top-0 bg-[#181a20] z-20 font-black uppercase tracking-widest text-[10px]">
                  <tr>
                    <th className="pb-4 pr-4">Symbol</th>
                    <th className="pb-4 pr-4">Type</th>
                    <th className="pb-4 pr-4 text-center">Volume (Lot)</th>
                    <th className="pb-4 pr-4 text-center">Profit / Loss</th>
                  ...
                  {bottomTab === 'TRADE' ? (
                    orders.map((o: any) => {
                      ...
                      return (
                        <tr key={o.id} className="hover:bg-white/[0.02] transition-colors group">
                          <td className="py-4 font-black uppercase text-xs sm:text-sm tracking-tighter">{o.symbol}</td>
                          ...
                          <td className={`py-4 text-center font-black font-mono text-[16px] ${pl >= 0 ? 'text-green-500' : 'text-red-400'}`}>
                            {pl >= 0 ? '+' : ''}{pl.toFixed(2)}
                          </td>
                          <td className="py-4 text-right">
                            <button 
                              onClick={() => closeOrder(o.id)} 
                              className="bg-red-500/10 text-red-500 hover:bg-red-600 hover:text-white px-6 py-2 rounded-xl font-black text-[10px] uppercase border border-red-500/20 transition-all shadow-lg active:scale-95"
                            >
                              Close
                            </button>
                          </td>
                    ...
                    history.filter((h: any) => h.marketType === MarketType.FOREX).map((h: any) => (
                      <tr key={h.id} className="border-b border-white/[0.02] opacity-60">
                        ...
                        <td className="py-4 text-center font-mono font-bold">{h.volume.toFixed(2)}</td>
                        <td className={`py-4 text-center font-black font-mono text-[16px] ${h.profit >= 0 ? 'text-green-500' : 'text-red-400'}`}>
                          {h.profit >= 0 ? '+' : ''}{h.profit.toFixed(2)}
                        </td>
                        <td className="py-4 text-right text-gray-600 font-mono italic text-[10px]">{new Date(h.timestamp).toLocaleDateString()}</td>
                      </tr>
                    ))
                  )}
                </tbody>
               ...
               {(bottomTab === 'TRADE' ? orders : history).length === 0 && (
                 <div className="p-20 text-center text-gray-800 font-black uppercase tracking-[0.4em] italic opacity-30">No Database Records Found</div>
               )}
            </div>
          </div>
        </div>
